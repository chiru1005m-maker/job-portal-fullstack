...
export default function Home() {
  ...
  // UPDATED: Now sends requirements to the backend search endpoint
  const fetchJobs = async () => {
    ...
    try {
      ...
      // Ensure your backend has @GetMapping("/search") in JobController
      const response = await api.get(`/api/jobs/search?${queryParams}`);
      const jobList = response.data.content ? response.data.content : response.data;
      setJobs(jobList);
    } catch (error) {
      console.error("Search failed:", error);
      setJobs([]); // Clear results if search fails
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      ...
      {/* Search Bar - Corrected button logic */}
      ...
      <div style={gridStyle}>
        {jobs.length > 0 ? (
          jobs.map(job => {
            const jobOwnerName = job.owner?.username || job.owner; 
              ...
              <div key={job.id} style={jobCard}>
                {isEditing ? (
                  ...
                  <>
                    ...
                    <h3 style={{ marginBottom: '10px', fontSize: '1.2rem' }}>{job.title}</h3>
...
const jobCard = { padding: '25px', border: '1px solid #eee', borderRadius: '15px', backgroundColor: 'white', position: 'relative', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', minHeight: '280px' };
